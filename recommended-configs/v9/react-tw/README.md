# Recommended ESLint Configuration for React + Tailwind Projects

> **This configuration is for ESLint v9.** Requires ESLint `>= 9.0.0` and `< 10.0.0`. For ESLint v10, see the [v10 configuration](../../v10/react-tw/).

This is the **recommended ESLint flat configuration** for React projects using Tailwind CSS (without TypeScript). This configuration provides the ultimate code style consistency by combining:
...
```bash
npm install --save-dev eslint@^9 @eslint/js @stylistic/eslint-plugin globals eslint-plugin-react eslint-plugin-react-hooks eslint-plugin-import-x eslint-plugin-jsx-a11y eslint-plugin-check-file eslint-plugin-perfectionist eslint-plugin-simple-import-sort eslint-plugin-tailwindcss eslint-plugin-code-style
```
...
| `import-x/no-unresolved` | `off` | Path aliases like `@/` cause false positives; rely on bundler for this |
...
The `react/jsx-filename-extension` rule is set to allow JSX in `.js` and `.jsx` files.

---

## Import Resolver Settings

The configuration includes JavaScript-aware import resolution:
...
| Feature | React Config | React + TW Config | React + TS + TW Config |
|---------|-------------|-------------------|------------------------|
| TypeScript Parser | ❌ | ❌ | ✅ |
| TypeScript Rules | ❌ | ❌ | ✅ |
| Tailwind Rules | ❌ | ✅ | ✅ |
| Interface Sorting | ❌ | ❌ | ✅ |
| File Extensions | `.js`, `.jsx` | `.js`, `.jsx` | `.js`, `.jsx`, `.ts`, `.tsx` |
